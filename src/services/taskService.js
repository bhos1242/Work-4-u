...
export async function updateTask(taskId, taskStatus, taskerEmail) {
  try {
...
export async function completeTask(taskId, taskStatus) {
  try {
    // Ensure status is set to "COMPLETED" before sending the update

    // Send the updated data to the server using Axios
    const response = await httpAxios.put(`/api/completeTask`, {
      taskId: taskId,
      taskStatus: taskStatus,
    });
