  ...
  const handleMakeOffer = async () => {
    try {
      if (taskData.status === "COMPLETED") {
        ...
        const shouldAcceptOffer = window.confirm(
          "Are you sure you want to accept this offer?"
        );

        if (shouldAcceptOffer) {
          // Get the tasker's email from the session
          const taskerEmail = session?.user?.email;

          // Update the task with the tasker's email
          const response = await updateTask(task_id, "ASSIGNED", taskerEmail);
          ...
          <div className="flex flex-col items-center justify-center p-6 bg-gray-200 rounded-lg md:w-1/3 lg:w-2/4 xl:w-2/4">
            ...
            <button
              ...
              disabled={
                taskData.status === "COMPLETED" ||
                taskData.status === "ASSIGNED"
              } // Disable the button if the task is completed or assigned
            >
              {taskData.status === "COMPLETED"
                ? "Task is Completed"
                : taskData.status === "ASSIGNED"
